...
* **The Ingestion & Caching Layer (OpenKB & OKF v0.1):** To establish an immutable foundation, foundational medical biology textbooks are pre-processed using **OpenKB**. OpenKB compiles long textbook PDFs via a hierarchical table of contents index into flat, cross-linked Markdown files backed by YAML frontmatter, complying strictly with the **Google Open Knowledge Format (OKF v0.1)** standard. This local knowledge base is queried first using semantic lookup tools, bypassing expensive external web lookups for static foundational terms.
* **The Data Control Plane (PubMed MCP Tool):** When dynamic, contemporary clinical research is requested, the system executes an automated tool call to the official **NCBI Entrez Programming Utilities (E-utilities) API**. The raw XML response is programmatically parsed using Python's native `xml.etree.ElementTree` utility to isolate the `<AbstractText>` node, stripping out metadata footnotes to optimize token context windows.

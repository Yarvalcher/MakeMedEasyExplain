...
### Phase 7: Callback-based Agent Input & Tool Guardrails
Enhance agent safety and control mechanisms using ADK lifecycle callbacks.

*   [ ] **Block 7.1: Pre-LLM Input Guardrail (`before_model_callback`)**
    ...
    *   **Testable Assertions (TDD)**:
        *   `test_input_guardrail_blocks_inappropriate_content()`: Verifies that policy-violating keywords result in an immediate refusal without calling the LLM.
*   [ ] **Block 7.2: Pre-Tool Execution Guardrail (`before_tool_callback`)**
    ...
    *   **Testable Assertions (TDD)**:
        *   `test_tool_guardrail_blocks_invalid_arguments()`: Verifies tool execution is bypassed when restricted arguments are passed.
